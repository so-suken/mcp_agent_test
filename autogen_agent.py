...
class AgentManager:
    """
    A class that manages a team of agents with a planner that coordinates tasks
    and uses Autogen with MCP tools to generate responses.
    """
    
    def __init__(self):
        """Initialize the agent manager with Azure OpenAI configuration"""
        ...
        # Create worker agents that will be available for tasks
        self.worker_agents = []
    ...
    async def initialize_agents(self):
        ...
        dialogue_tools = await self.get_dialogue_tools()
        if dialogue_tools:
            ...
            self.worker_agents.append(dialogue_agent)
            print("Dialogue agent initialized with tools")
        
        # Create PostgreSQL agent with tools
        try:
            postgres_tools = await self.get_postgres_tools()
            if postgres_tools:
                ...
                self.worker_agents.append(postgres_agent)
        ...
        self.worker_agents.append(formatter_agent)
        print("Formatter agent initialized")
        
        # Create agents description for planner
        agents_description = "\n".join([f"{agent.name}: {agent.description}" for agent in self.worker_agents])
        ...
        # Add planner to the list of all agents
        self.all_agents = self.worker_agents + [self.planner]
    
    async def get_dialogue_tools(self) -> List:
    ...
    async def get_postgres_tools(self) -> List:
    ...
    def create_termination_condition(self, keyword: str = "[TERMINATE]", max_turns: int = 15, max_time: int = 300):
    ...
    async def process_query(self, query: str) -> str:
        ...
        try:
            ...
            if not self.worker_agents:
                return "Error: No worker agents available. Check the logs for initialization errors."
            
            # Create the SelectorGroupChat
            chat = SelectorGroupChat(
                participants=self.all_agents,
                model_client=self.model_client,
    ...
    # Helper function to extract content from various response types
    def extract_content(self, response, default_message="No response content available"):
